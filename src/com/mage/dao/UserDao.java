...
public class UserDao {
	...
	public static User queryUserByName(String uname) {
		...
		try {
			...
			while(res.next()) {
				...
				user.setBankid(res.getString("bankid"));
	...
	public int register(User adduser) {
		...
		try {
			conn=DBUtil.getConnection();
			String sql="insert into t_user (address,birthday,dentityCode,email,mobile,password,sex,userName,status,trueName,bankid)"
					+ " values (?,?,?,?,?,?,?,?,?,?,?)";
			sta=conn.prepareStatement(sql);
			//设置参数
			...
			sta.setString(11, adduser.getBankid());
